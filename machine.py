...
def display_menu(menu):
    ...
    if choice > len(options)-1 or choice < 0:
        print(f"Wprowadzono błędny numer opcji")
        return display_menu(menu)
...
def register_payment(cart):
    ...
    if payment_method.lower() == "b":
        ...
        if re.search(r"^[0-9]{6}$",blik):
            print(f"transakcja się powiodłą")
        else:
            decision = input("Podano niepoprawny kod. Czy chcesz sprobować dokonać platności (t/n)?")
            if decision.lower() == "t":
        ...
        print(f"Proszę zbliżyć kartę do czytnika...")
        input("Potwierdż transakcje: ")
        print(f"Transakcja się powiodłą")
        ...
        while paid < total:
            try:
                ...
                paid += 0
            if paid < total:
                decision = input("Wprowadzono za mało gotówki. Czy chcesz doplacić (t/n)?")
                if decision.lower() == "t":
                    continue
                else: exit()
        if paid > total:
        ...
        decision = input("Wybrano niepoprawną opcje. Czy chcesz spróbować jeszcze raz(t/n)?")
        if decision.lower() == "t":
            return register_payment(cart)
        else:
            exit()
...
register_payment(cart)
input(f"Drukowanie biletów. Proszę czekać ...")
display_cart(cart)
print(f"Dziękujemy za skorzustanie z automatu biletowego. Zapraszamy ponownie")
